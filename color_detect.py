...
sender_address = 'fasalmvj@gmail.com'
sender_pass = 'papamummy1998'                                                         #information for email
...
while(1):
    ...
    for pic, contour in enumerate(contours):
        ...
        if(area>3000):
            ...
            cv2.putText(img,"  ",(x, y),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
    ...
    for pic, contour in enumerate(contours):
        ...
        if(area>3000):
            ...
            cv2.putText(img,"  ",(x,y),cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,0,0))  
    ...
    if cv2.waitKey(5) & 0xFF==ord('a'):
        ...
        for i in range(len(report)):
            ...
            df1.loc[6]=(date[10:16],report[i])
            df1.to_csv(lis[i]+".csv",index=False)  
        
            
    if  cv2.waitKey(2) & 0xFF == ord('y'):
       # p = bokeh.figure(x_range=lis, plot_height=250, title=new_c)
       # p.vbar(x=lis, top=report, width=0.9)
        ...
        session.starttls()                                                               
        session.login('fasalmvj@gmail.com', 'papamummy1998')                                       
        text = message.as_string()
        session.sendmail('fasalmvj@gmail.com', 'tarun121919982000@gmail.com', text)
        session.quit()
        print("done")
